...
function _GF_veh_hijack(_veh,_bool)
	...
	if _GF_valid_veh(_veh) then
		...
		if _bool and _GF_is_ent(vehicle.get_ped_in_vehicle_seat(_veh, -1)) and ped.is_ped_a_player(vehicle.get_ped_in_vehicle_seat(_veh, -1)) and vehicle.get_ped_in_vehicle_seat(_veh, -1) ~= _GT_ME._ped() then
			time = utils.time_ms() + 1500
			_GF_veh_kick_script(player.get_player_from_ped(vehicle.get_ped_in_vehicle_seat(_veh, -1)))
		end
		while _GF_is_ent(vehicle.get_ped_in_vehicle_seat(_veh, -1)) and vehicle.get_ped_in_vehicle_seat(_veh, -1) ~= _GT_ME._ped() and utils.time_ms() < time do
			ped.clear_ped_tasks_immediately(vehicle.get_ped_in_vehicle_seat(_veh, -1))
			system.yield(0)
		end
		if not _GF_is_ent(vehicle.get_ped_in_vehicle_seat(_veh, -1)) then
			time = utils.time_ms() + 100
			while time > utils.time_ms() and vehicle.get_ped_in_vehicle_seat(_veh, -1) ~= _GT_ME._ped() do
				ped.set_ped_into_vehicle(_GT_ME._ped(),_veh,-1)	
				system.yield(10)
			end
		end
		return vehicle.get_ped_in_vehicle_seat(_veh, -1) == _GT_ME._ped()
	end
	return false
end
...
function _GF_tp_into_free_seat(_veh)
	if _GF_valid_veh(_veh) then
		for i = 1,_GF_veh_seats(_veh) do
			if not entity.is_entity_a_ped(vehicle.get_ped_in_vehicle_seat(_veh, i-2)) then
				local time = utils.time_ms() + 100
				while time > utils.time_ms() and vehicle.get_ped_in_vehicle_seat(_veh, i-2) ~= _GT_ME._ped() do
					ped.set_ped_into_vehicle(_GT_ME._ped(),_veh,i-2)	
					system.yield(10)
				end
				if vehicle.get_ped_in_vehicle_seat(_veh, i-2) == _GT_ME._ped() then
					break
				end
...
function _GF_tp_personal_veh(_action)
	...
	elseif veh == my_veh and _GF_me_in_veh() then
...
function _GF_tp_last_veh(_action)
	local _veh
	if _GF_me_in_veh() then	_veh = _GT_my_veh_hist[#_GT_my_veh_hist-1] else	_veh = _GT_my_veh_hist[#_GT_my_veh_hist] end
