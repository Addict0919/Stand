...
function _GF_closest_ped(_plyr,_npc,_dead,_pos,_dist_pos)
	local all_peds=ped.get_all_peds()
	local PED=nil
	_GF_sort_xyz_ent(all_peds,_pos)
	for i=1,#all_peds do
		if entity.is_entity_dead(all_peds[i]) == _dead then
			if _plyr and _npc then
				PED = all_peds[i]
				break
			elseif _plyr and ped.is_ped_a_player(all_peds[i]) then
				PED = all_peds[i]
				break
			elseif _npc and not ped.is_ped_a_player(all_peds[i]) then
				PED = all_peds[i]
				break
			end
		end
	...
	if _dist_pos ~= nil then
		return PED,_GF_dist_pos_pos(_dist_pos,entity.get_entity_coords(PED))
	end
	return PED
end

function _GF_non_plyr_ped(_bool)
	_bool = _bool or false
	local all_peds=ped.get_all_peds()
	local non_plyr_ped,count = 100000,0
	if _bool then
		_GF_sort_xyz_ent(all_peds,player.get_player_coords(player.player_id()))
		for i=1,#all_peds do
...
function _GF_vector_point_to_pos(_1stpos,_2ndpos,_check_pos,_limit)
	local vector = _GF_vector_to_pos3(_2ndpos,_1stpos)
	local vector_check = _GF_vector_to_pos3(_check_pos,_1stpos)
	return _GF_v2_compare(v2(vector.x,vector.z),v2(vector_check.x,vector_check.z),_limit)
end

function _GF_v3_rot_compare_single(_1,_2)
	if (_1>=0 and _2>=0) or (_1<0 and _2<0) then
		return math.abs(_2-_1)
	end
	return math.abs(math.abs(_2-_1)-100)
end

function _GF_v3_rot_compare_total(_rot1,_rot2)
	return (_GF_v3_rot_compare_single(_rot1.x,_rot2.x)+_GF_v3_rot_compare_single(_rot1.y,_rot2.y)+_GF_v3_rot_compare_single(_rot1.z,_rot2.z))
end
...
function _GF_vector_to_pos3(_target_pos,_start_pos,_return_dist)
	local x = _target_pos.x - _start_pos.x
	local y = _target_pos.y - _start_pos.y
	local heading = math.atan(x, y) * -180 / math.pi    
	local z_dif = (_start_pos.z - _target_pos.z)*-1
	local dist = _GF_dist_pos_pos(_target_pos,_start_pos)
	local pitch = math.asin(z_dif/dist)/(2*math.pi)*360
...
function _GF_raycast_vector(_ign_ent,_only_ent,_pos,_rot)
	...
	local function get_my_pos_rot()
		local new_pos = _pos or cam.get_gameplay_cam_pos()
		local me_pos = new_pos
		local rot = _rot or cam.get_gameplay_cam_rot()
...
function _GF_front_of_dir(_pos,_rot,_dist)
	local new_pos = _pos
	local new_rot = _rot
...
function _GT_ME._ped()
	return player.get_player_ped(player.player_id())
end

function _GT_ME._veh()
	return player.get_player_vehicle(player.player_id())
end

function _GT_ME._crds()
	return player.get_player_coords(player.player_id())
end

function _GT_ME._head()
	return player.get_player_heading(player.player_id())
end

function _GT_ME._weap()
	return ped.get_current_ped_weapon(player.get_player_ped(player.player_id()))
end

function _GT_ME._veh_crds()
	return entity.get_entity_coords(player.get_player_vehicle(player.player_id()))
end

function _GT_ME._aim()
	return (player.is_player_free_aiming(player.player_id()) or _GF_key_active(25,1)) --aiming the homing missile isnt "freeaiming"
end

function _GT_ME._aircrft_shoot()
	if _GF_me_in_veh() and (streaming.is_model_a_heli(entity.get_entity_model_hash(_GT_ME._veh())) or streaming.is_model_a_plane(entity.get_entity_model_hash(_GT_ME._veh()))) then
		return (_GF_key_active(114,1.0) or _GF_key_active(331,1.0) or _GF_key_active(330,1.0))
	end
	return false
end

function _GT_ME._shoot()
	return ped.is_ped_shooting(player.get_player_ped(player.player_id()))
end

function _GF_total_velocity(_ent)
	local vel = entity.get_entity_velocity(_ent)
...
function _GF_GW_no_interfere()
	if _G_watch_dog.on then
		if _GF_key_active(25,0) then --prevents you from using wrong commands when using geewatch
			return true
		end
		return false
	end
...
function _GF_no_right_click(_time) --shit dont work
	...
	while utils.time_ms() < time do
		system.yield(0)
		controls.set_control_normal(0,25,0.0)
	end
end
...
function _GF_geeyee_calc2(_shotgun,_trgt_crds)
	_shotgun = _shotgun or false
	local v3_start=_GF_geeeye_start()
	...
	if not _shotgun then
		...
		local spread1000,spreadv3 = math.floor(_G_GE_main.spread.value*20),v3()
		if _G_GE_main.spread_type.value == 0 then -- circular
			spreadv3=v3(math.random(-spread1000,spread1000)*0.001,0,math.random(-spread1000,spread1000)*0.001)
		elseif _G_GE_main.spread_type.value == 1 then -- horizontal
			spreadv3=v3(0,0,math.random(-spread1000,spread1000)*0.001)
		else -- vertical
			spreadv3=v3((math.random(-spread1000,spread1000)*0.001)+3,0,0)
		end
		if _trgt_crds == nil then
			local dir = cam.get_gameplay_cam_rot()+spreadv3
			dir:transformRotToDir()
			dir = dir * 1500
			v3_end = v3_end + dir
		else
			local dir,dist = _GF_vector_to_pos3(_trgt_crds,v3_start,true)
			dir=dir+spreadv3
			dir:transformRotToDir()
			dir = dir * dist
			v3_end = v3_end + dir
...
function _GF_5_decimals(_value)
	return math.floor(_value) + math.floor((_value - math.floor(_value)) * 100000) * .00001
end

function _GF_4_decimals(_value)
	return math.floor(_value) + math.floor((_value - math.floor(_value)) * 10000) * .0001
end

function _GF_3_decimals(_value)
	return math.floor(_value) + math.floor((_value - math.floor(_value)) * 1000) * .001
end
	...
	if _GF_valid_veh(_veh) then
		...
		elseif _seat == "first_plyr" then
			if _GF_any_plyr_in_veh(_veh) then
				for i=1,_GF_veh_seats(_veh) do
					name = _GF_plyr_name(vehicle.get_ped_in_vehicle_seat(_veh, i-2))
...
function _GF_req_anim(_anim)
    while not streaming.has_anim_dict_loaded(_anim) do
      streaming.request_anim_dict(_anim)
      system.yield(10)
    end
end
...
function _GF_give_ped_weap(_ped,_weapon_hash)
	...
	if good and _GF_good_ped(_ped,nil,nil,nil) then
		if not weapon.has_ped_got_weapon(_ped, _weap) then
			weapon.give_delayed_weapon_to_ped(_ped, _weap, 1, 1)
		end
		system.yield(5)
		_GF_upgrade_weap(_ped,_weap)
		_GF_give_ped_ammo(_ped,_weap)
		return true
	end
	return false
end

function _GF_upgrade_weap(_ped,_weap)
	local found = false
	local function give_comp(_type,_bool)
		if _type[_weap]~= nil then
			weapon.give_weapon_component_to_ped(_ped,_weap, _type[_weap])
			if _bool then found=true end
		end
		return found
	end
	give_comp(_GT_weap_comp.flshlght,false)
	give_comp(_GT_weap_comp.scope,false)
	give_comp(_GT_weap_comp.grip,false)
	if not give_comp(_GT_weap_comp.ammo.explsv,true) then
		if not give_comp(_GT_weap_comp.ammo.fire,true) then
			if not give_comp(_GT_weap_comp.ammo.armr,true) then
				if not give_comp(_GT_weap_comp.ammo.fmj,true) then
					give_comp(_GT_weap_comp.ammo.hllw,true)
				end
			end
		end
	end
	if not found then
		give_comp(_GT_weap_comp.mag,false)
	end
	found=false
	give_comp(_GT_weap_comp.spprsr,true)
	if not found then
		give_comp(_GT_weap_comp.brake,true)
	end
	weapon.set_ped_weapon_tint_index(_ped, _weap, math.random(1, weapon.get_weapon_tint_count(_weap)))
end
	

function _GF_give_ped_ammo(_ped,_weapon_hash)
	local found, ammo = weapon.get_max_ammo(_ped,_weapon_hash)
	local time = utils.time_ms() + 10
	while time > utils.time_ms() and not found do
		system.yield(0)
		found, ammo = weapon.get_max_ammo(_ped,_weapon_hash)
	end
	if found then
		weapon.set_ped_ammo(_ped, _weapon_hash,ammo)
		return true
	end
	return false
end
...
function _GF_any_ped_in_veh(_veh)
	for i = 1,_GF_veh_seats(_veh) do
		if _GF_is_ent(vehicle.get_ped_in_vehicle_seat(_veh, i-2)) then
			return true
		end
	end
	return false
...
function _GF_me_in_veh()
    return (player.is_player_in_any_vehicle(player.player_id()) and _GF_valid_veh(player.get_player_vehicle(player.player_id())))
end
...
function _GF_rpr_basic(aim)
	...
	native.call(0x31B927BBC44156CD,aim,1)--SetVehicleIsConsideredByPlayer
...
function _GF_veh_destroyed_do(_veh, _pid)
	native.call(0x31B927BBC44156CD,_veh,0)--SetVehicleIsConsideredByPlayer
...
_GT_weap_comp={}
_GT_weap_comp.mag={
[gameplay.get_hash_key("weapon_heavyrifle")] = gameplay.get_hash_key("COMPONENT_HEAVYRIFLE_CLIP_02"),
[gameplay.get_hash_key("weapon_emplauncher")] = gameplay.get_hash_key("COMPONENT_EMPLAUNCHER_CLIP_01"),
[gameplay.get_hash_key("weapon_pistol")] = gameplay.get_hash_key("COMPONENT_PISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_combatpistol")] = gameplay.get_hash_key("COMPONENT_COMBATPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_appistol")] = gameplay.get_hash_key("COMPONENT_APPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_pistol50")] = gameplay.get_hash_key("COMPONENT_PISTOL50_CLIP_02"),
[gameplay.get_hash_key("weapon_snspistol")] = gameplay.get_hash_key("COMPONENT_SNSPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_heavypistol")] = gameplay.get_hash_key("COMPONENT_HEAVYPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_SNSPISTOL_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_PISTOL_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_vintagepistol")] = gameplay.get_hash_key("COMPONENT_VINTAGEPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_ceramicpistol")] = gameplay.get_hash_key("COMPONENT_CERAMICPISTOL_CLIP_02"),
[gameplay.get_hash_key("weapon_microsmg")] = gameplay.get_hash_key("COMPONENT_MICROSMG_CLIP_02"),
[gameplay.get_hash_key("weapon_smg")] = gameplay.get_hash_key("COMPONENT_SMG_CLIP_03"),
[gameplay.get_hash_key("weapon_assaultsmg")] = gameplay.get_hash_key("COMPONENT_ASSAULTSMG_CLIP_02"),
[gameplay.get_hash_key("weapon_minismg")] = gameplay.get_hash_key("COMPONENT_MINISMG_CLIP_02"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_SMG_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_machinepistol")] = gameplay.get_hash_key("COMPONENT_MACHINEPISTOL_CLIP_03"),
[gameplay.get_hash_key("weapon_combatpdw")] = gameplay.get_hash_key("COMPONENT_COMBATPDW_CLIP_03"),
[gameplay.get_hash_key("weapon_assaultshotgun")] = gameplay.get_hash_key("COMPONENT_ASSAULTSHOTGUN_CLIP_02"),
[gameplay.get_hash_key("weapon_heavyshotgun")] = gameplay.get_hash_key("COMPONENT_HEAVYSHOTGUN_CLIP_03"),
[gameplay.get_hash_key("weapon_assaultrifle")] = gameplay.get_hash_key("COMPONENT_ASSAULTRIFLE_CLIP_03"),
[gameplay.get_hash_key("weapon_carbinerifle")] = gameplay.get_hash_key("COMPONENT_CARBINERIFLE_CLIP_03"),
[gameplay.get_hash_key("weapon_advancedrifle")] = gameplay.get_hash_key("COMPONENT_ADVANCEDRIFLE_CLIP_02"),
[gameplay.get_hash_key("weapon_specialcarbine")] = gameplay.get_hash_key("COMPONENT_SPECIALCARBINE_CLIP_03"),
[gameplay.get_hash_key("weapon_bullpuprifle")] = gameplay.get_hash_key("COMPONENT_BULLPUPRIFLE_CLIP_02"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_BULLPUPRIFLE_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_SPECIALCARBINE_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_ASSAULTRIFLE_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_CARBINERIFLE_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_compactrifle")] = gameplay.get_hash_key("COMPONENT_COMPACTRIFLE_CLIP_03"),
[gameplay.get_hash_key("weapon_militaryrifle")] = gameplay.get_hash_key("COMPONENT_MILITARYRIFLE_CLIP_02"),
[gameplay.get_hash_key("weapon_mg")] = gameplay.get_hash_key("COMPONENT_MG_CLIP_02"),
[gameplay.get_hash_key("weapon_combatmg")] = gameplay.get_hash_key("COMPONENT_COMBATMG_CLIP_02"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_COMBATMG_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_gusenberg")] = gameplay.get_hash_key("COMPONENT_GUSENBERG_CLIP_02"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_MARKSMANRIFLE_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_02"),
[gameplay.get_hash_key("weapon_marksmanrifle")] = gameplay.get_hash_key("COMPONENT_MARKSMANRIFLE_CLIP_02"),
}

_GT_weap_comp.ammo={}
_GT_weap_comp.ammo.explsv={
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_PUMPSHOTGUN_MK2_CLIP_EXPLOSIVE"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_EXPLOSIVE"),
}
_GT_weap_comp.ammo.fire={	
[gameplay.get_hash_key("weapon_revolver_mk2")] = gameplay.get_hash_key("COMPONENT_REVOLVER_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_SNSPISTOL_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_PISTOL_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_SMG_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_PUMPSHOTGUN_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_BULLPUPRIFLE_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_SPECIALCARBINE_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_ASSAULTRIFLE_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_CARBINERIFLE_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_COMBATMG_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_MARKSMANRIFLE_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_INCENDIARY"),
}

_GT_weap_comp.ammo.fmj={
[gameplay.get_hash_key("weapon_revolver_mk2")] = gameplay.get_hash_key("COMPONENT_REVOLVER_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_SNSPISTOL_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_PISTOL_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_SMG_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_BULLPUPRIFLE_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_SPECIALCARBINE_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_ASSAULTRIFLE_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_CARBINERIFLE_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_COMBATMG_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_MARKSMANRIFLE_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_FMJ"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_INCENDIARY"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_INCENDIARY"),
}

_GT_weap_comp.ammo.hllw={
[gameplay.get_hash_key("weapon_revolver_mk2")] = gameplay.get_hash_key("COMPONENT_REVOLVER_MK2_CLIP_HOLLOWPOINT"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_SNSPISTOL_MK2_CLIP_HOLLOWPOINT"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_PISTOL_MK2_CLIP_HOLLOWPOINT"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_SMG_MK2_CLIP_HOLLOWPOINT"),
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_PUMPSHOTGUN_MK2_CLIP_HOLLOWPOINT"),
}

_GT_weap_comp.ammo.armr={
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_BULLPUPRIFLE_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_SPECIALCARBINE_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_ASSAULTRIFLE_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_CARBINERIFLE_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_COMBATMG_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_MARKSMANRIFLE_MK2_CLIP_ARMORPIERCING"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_HEAVYSNIPER_MK2_CLIP_ARMORPIERCING"),
}

_GT_weap_comp.flshlght={
[gameplay.get_hash_key("weapon_heavyrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_pistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_combatpistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_appistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_pistol50")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_heavypistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_revolver_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH_03"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH_02"),
[gameplay.get_hash_key("weapon_microsmg")] = gameplay.get_hash_key("COMPONENT_AT_PI_FLSH"),
[gameplay.get_hash_key("weapon_smg")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_assaultsmg")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_combatpdw")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_assaultshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_bullpupshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_heavyshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_combatshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_assaultrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_carbinerifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_advancedrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_specialcarbine")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_bullpuprifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_militaryrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_marksmanrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_grenadelauncher")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
[gameplay.get_hash_key("weapon_grenadelauncher_smoke")] = gameplay.get_hash_key("COMPONENT_AT_AR_FLSH"),
}

_GT_weap_comp.scope={
[gameplay.get_hash_key("weapon_heavyrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM"),
[gameplay.get_hash_key("weapon_revolver_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SIGHTS"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_RAIL_02"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_RAIL"),
[gameplay.get_hash_key("weapon_microsmg")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MACRO"),
[gameplay.get_hash_key("weapon_smg")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MACRO_02"),
[gameplay.get_hash_key("weapon_assaultsmg")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MACRO"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SIGHTS_SMG"),
[gameplay.get_hash_key("weapon_combatpdw")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SIGHTS"),
[gameplay.get_hash_key("weapon_assaultrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MACRO"),
[gameplay.get_hash_key("weapon_carbinerifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM"),
[gameplay.get_hash_key("weapon_advancedrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_specialcarbine")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM"),
[gameplay.get_hash_key("weapon_bullpuprifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL_MK2"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM_MK2"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM_MK2"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM_MK2"),
[gameplay.get_hash_key("weapon_militaryrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_mg")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL_02"),
[gameplay.get_hash_key("weapon_combatmg")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MEDIUM_MK2"),
[gameplay.get_hash_key("weapon_sniperrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MAX"),
[gameplay.get_hash_key("weapon_heavysniper")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_MAX"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_LARGE_FIXED_ZOOM_MK2"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_THERMAL"),
[gameplay.get_hash_key("weapon_marksmanrifle")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_LARGE_FIXED_ZOOM"),
[gameplay.get_hash_key("weapon_grenadelauncher")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_grenadelauncher_smoke")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
}

_GT_weap_comp.spprsr={
[gameplay.get_hash_key("weapon_heavyrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_pistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP_02"),
[gameplay.get_hash_key("weapon_combatpistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_appistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_pistol50")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_heavypistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_snspistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP_02"),
[gameplay.get_hash_key("weapon_pistol_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP_02"),
[gameplay.get_hash_key("weapon_vintagepistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_ceramicpistol")] = gameplay.get_hash_key("COMPONENT_CERAMICPISTOL_SUPP"),
[gameplay.get_hash_key("weapon_microsmg")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_smg")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_assaultsmg")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_smg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_machinepistol")] = gameplay.get_hash_key("COMPONENT_AT_PI_SUPP"),
[gameplay.get_hash_key("weapon_combatpdw")] = gameplay.get_hash_key("COMPONENT_AT_SCOPE_SMALL"),
[gameplay.get_hash_key("weapon_pumpshotgun")] = gameplay.get_hash_key("COMPONENT_AT_SR_SUPP"),
[gameplay.get_hash_key("weapon_assaultshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_bullpupshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_pumpshotgun_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SR_SUPP_03"),
[gameplay.get_hash_key("weapon_heavyshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_combatshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_assaultrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_carbinerifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_advancedrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_specialcarbine")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_bullpuprifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_militaryrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_sniperrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP_02"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
[gameplay.get_hash_key("weapon_heavysniper_mk2")] = gameplay.get_hash_key("COMPONENT_AT_SR_SUPP_03"),
[gameplay.get_hash_key("weapon_marksmanrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_SUPP"),
}

_GT_weap_comp.brake={
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_MUZZLE_07"),
}

_GT_weap_comp.grip={
[gameplay.get_hash_key("weapon_heavyrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_combatpdw")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_assaultshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_bullpupshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_heavyshotgun")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_assaultrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_carbinerifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_specialcarbine")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_bullpuprifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_bullpuprifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_specialcarbine_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_assaultrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_carbinerifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_combatmg")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_combatmg_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_marksmanrifle_mk2")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP_02"),
[gameplay.get_hash_key("weapon_marksmanrifle")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_grenadelauncher")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
[gameplay.get_hash_key("weapon_grenadelauncher_smoke")] = gameplay.get_hash_key("COMPONENT_AT_AR_AFGRIP"),
