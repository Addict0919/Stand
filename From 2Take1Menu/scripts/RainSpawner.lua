      ...
      if request_model(model_name) == true then
        if streaming.is_model_a_vehicle(Veh_Hash) then
          for i = 1, spawnAmountV do
            ...
            ui.notify_above_map("Rained " ..model_name.. " For " ..player.get_player_name(pid), "Successful", 210)
          else
      ...
      if not cooldown then
          ...
          if request_model(model_name) == true then
            if streaming.is_model_a_ped(Ped_Hash) then
              ...
              ui.notify_above_map("Rained " ..model_name.. " For " ..player.get_player_name(pid), "Successful", 210)
              ui.notify_above_map("Clearing peds in 15 seconds, there will be a cooldown", "Clearing", 210)
              cooldown = true
              syswait(10000)
              cooldown = false
