...
local function init()
	stand.action(stand.my_root(), "Reset State", {"resetstate"}, "", function()
	...
	local settings_list = stand.list(stand.my_root(), "Settings", {"2t1settings"}, "")
	stand.toggle(settings_list, "Spoof 2Take1Menu Directory", {"spoof2t1load"}, "Tries to redirect access to resources from the 2Take1Menu directory to the \"From 2Take1Menu\" folder.", function (value)
		config.spoof_2take1_install_dir = value
	end, true)

	stand.trigger_commands("spoof2t1load")

	local no_scripts = true
	local lua_list = stand.list(stand.my_root(), "Load Scripts", {"loadscripts"}, "", function()
		if no_scripts then
